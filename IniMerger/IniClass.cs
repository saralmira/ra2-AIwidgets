    ...
    public class IniClass
    {
        ...
        public void WriteValue(string section, string key, string value)
        {
            ...
            else if (key != null)
            {
                ...
                sectioninf.Add(new Section { keys = new List<string> { key }, values = new List<string> { value } });
            }
        }
        ...
        public struct Section
        {
            ...
            public static bool operator ==(Section Asec, Section Bsec)
            {
                List<string> A = new List<string>();
                List<string> B = new List<string>();
                for (int i = 0; i < Asec.keys.Count; ++i)
                {
                    if (!Asec.isnote[i])
                        A.Add(Asec.keys[i]);
                }
                for (int i = 0; i < Bsec.keys.Count; ++i)
                {
                    if (!Bsec.isnote[i])
                        B.Add(Bsec.keys[i]);
                }
                return A.Count == B.Count && A.TrueForAll(B.Contains);
            }
